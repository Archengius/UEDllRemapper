...
) 
{
	...
	else 
	{
		...
		case SymTagTypedef:
            wcstombs(buffer, Info.Info.sTypedefInfo.Name, 500);
			TypeName += buffer;
			break; 
		...
		case SymTagUDT: 
			{
				...
				{
					// A class/structure
                    wcstombs(buffer, Info.Info.sUdtClassInfo.Name, 500);
					TypeName += buffer;
                    ...
                    // A union
                    wcstombs(buffer, Info.Info.sUdtUnionInfo.Name, 500);
                    TypeName += buffer;
                }
			}
			break; 
		...
		case SymTagEnum:
            wcstombs(buffer, Info.Info.sEnumInfo.Name, 500);
			TypeName += buffer;
			break; 
...
LPCTSTR CTypeInfoText::DataKindStr( SYMBOL_INFO& rSymbol ) 
{
	...
	if( !SymGetTypeInfo( GetCurrentProcess(), rSymbol.ModBase, rSymbol.TypeIndex, TI_GET_DATAKIND, &Dk ) )
	{
